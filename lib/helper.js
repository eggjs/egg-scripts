...
exports.findfindNodeProcessWin = function* (argv) {
  ...
  if (keyStr) { // title match first
    ...
    workerPids = yield findWorkerPidsWin(keyStr);
    ...
    workerPids = yield findWorkerPidsByMaster(masterPids[0]);
  }
 ...
 * @return {number[]} - array pids of app/agent work
 */
...
function* findWorkerPidsWin(str) {
  ...
  const command = `tasklist /FI "WINDOWTITLE eq ${keyStr}"`;
 ...
 *   cmd: string    // 'E:/.../node_modules/egg-cluster/lib/agent_worker.js'
