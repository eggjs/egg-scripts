...
class StartCommand extends Command {
  ...
  * run(context) {
    ...
    if (isDaemon) {
      ...
      debug('Run spawn `%s %s`', options.node, eggArgs.join(' '));
      const child = this.child = spawn(options.node, eggArgs, options);
      this.isReady = false;
      ...
      child.once('exit', code => {
        if (code !== 0) {
          child.emit('error', new Error(`spawn ${options.node} ${eggArgs.join(' ')} fail, exit code: ${code}`));
        }
      });
