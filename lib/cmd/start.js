...
class StartCommand extends Command {
  ...
  * run(context) {
    ...
    const HOME = homedir();
    let logDir = path.join(HOME, 'logs');
    ...
    // home/pkginfo.name -> config.Logger.dir
    logDir = path.join(logDir, pkgInfo.name);
    if (logDir !== this.config.logger.dir) {
      logDir = this.config.logger.dir;
      this.logger.info('Set application log file dir %s', this.config.logger.dir);
    }
