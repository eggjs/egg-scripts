...
class StopCommand extends Command {
  ...
  * run(context) {
    ...
    const processList = yield this.helper.findProcess('name', 'node');
    let pids = processList.filter(item => {
      const cmd = item.cmd;
      if (process.platform === 'win32') {
        console.log('>', cmd.includes(JSON.stringify(`"baseDir":"${baseDir}"`)), JSON.stringify(`"baseDir":"${baseDir}"`));
        console.log(cmd);
        return false;
