...
class StopCommand extends Command {
  ...
  * run(context) {
    ...
    let processList = yield this.helper.findNodeProcess(item => {
      const cmd = item.cmd;
      return cmd.includes('start-cluster') && cmd.includes(`"title":"${argv.title}"`);
    });
    ...
    processList = yield this.helper.findNodeProcess(item => {
      const cmd = item.cmd;
      return cmd.includes(`"title":"${argv.title}"`) && (cmd.includes('egg-cluster/lib/app_worker.js') || cmd.includes('egg-cluster/lib/agent_worker.js'));
    });
