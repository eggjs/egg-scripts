...
class StopCommand extends Command {
  ...
  * run(context) {
    ...
    let processList = yield this.helper.findNodeProcess(item => {
      ...
      return cmd.includes('start-cluster') && cmd.includes('baseDir') && cmd.includes(baseDir.replace(/\\/g, '/'));
    });
    let pids = processList.map(x => x.pid);

    yield this.helper.getEggMaster(baseDir);
    ...
    processList = yield this.helper.findNodeProcess(item => {
      const cmd = item.cmd;
      return (cmd.includes('app_worker.js') || cmd.includes('agent_worker.js')) && cmd.includes('baseDir') && cmd.includes(baseDir.replace(/\\/g, '/'));
    });
